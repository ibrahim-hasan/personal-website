...
class EditorialArticleRelationsApiTest extends TestCase
{
    ...
    public function test_repeated_updates_can_retain_public_targets_while_the_article_is_still_a_draft(): void
    {
        $service = Service::factory()->create(['key' => 'repeatable-service']);
        $project = $this->eligibleProject('repeatable-project');
        $article = Article::factory()->create(['is_published' => false, 'editorial_revision' => 1]);

        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-repeat-service-001',
                'If-Match' => '"1"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
        ...
        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-repeat-service-002',
                'If-Match' => '"2"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
                'service_keys' => [$service->key],
        ...
        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-repeat-project-001',
                'If-Match' => '"3"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
        ...
        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-repeat-project-002',
                'If-Match' => '"4"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
                'project_keys' => [$project->key],
    ...
    public function test_relation_updates_reject_targets_with_another_draft_article(): void
    {
        ...
        $otherDraft = Article::factory()->create(['is_published' => false]);
        $otherDraft->services()->attach($service, ['sort_order' => 0]);
        $otherDraft->projects()->attach($project, ['sort_order' => 0]);
        $article = Article::factory()->create(['is_published' => false, 'editorial_revision' => 1]);

        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-other-draft-service',
                'If-Match' => '"1"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
                'service_keys' => [$service->key],
        ...
        $this->asClient(['articles:write'])
            ...
            ->assertUnprocessable()
            ->assertJsonValidationErrors(['project_keys.0']);
    }

    public function test_an_update_can_clear_supplied_relation_sets(): void
    {
        $service = Service::factory()->create(['key' => 'clearable-service']);
        $project = $this->eligibleProject('clearable-project');
        $article = Article::factory()->create(['is_published' => false, 'editorial_revision' => 1]);

        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-clear-setup',
                'If-Match' => '"1"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
                'service_keys' => [$service->key],
        ...
        $this->asClient(['articles:write'])
            ->withHeaders([
                'Idempotency-Key' => 'article-relations-clear-sets',
                'If-Match' => '"2"',
            ])
            ->patchJson('/api/v1/articles/'.$article->getKey(), [
                'service_keys' => [],
            ...
            ->assertOk()
            ->assertJsonPath('data.service_keys', [])
            ->assertJsonPath('data.project_keys', []);

        $article->refresh();

        $this->assertSame([], $article->relatedServiceKeys());
        $this->assertSame([], $article->relatedProjectKeys());
        $snapshot = $article->revisionSnapshots()->where('revision', 3)->sole();

        $this->assertSame([], $snapshot->service_keys);
        $this->assertSame([], $snapshot->project_keys);
    }
    ...
    public function test_unknown_duplicate_and_non_public_related_content_keys_are_rejected_with_field_specific_errors(): void
    {
        ...
        $payload['project_keys'] = ['missing-project'];
