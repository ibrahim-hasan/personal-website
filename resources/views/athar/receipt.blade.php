    ...
    <section class="athar-panel athar-panel--center" aria-labelledby="athar-receipt-title">
        ...
        @if (isset($version) && in_array($versionStatus, ['draft', 'awaiting_approval'], true))
            @php
                ...
                $identityDisplay = old('identity_display', $pendingInput['identity_display'] ?? ($payload['identity_display'] ?? 'full_name'));
                $displayName = old('display_name', $pendingInput['display_name'] ?? ($payload['display_name'] ?? ($version->display_name ?? $invitation->recipient_name ?? '')));
                $displayPosition = old('display_position', $pendingInput['display_position'] ?? ($payload['display_position'] ?? ($invitation->recipient_position ?? '')));
            @endphp
            <div class="athar-final-preview-wrap" x-data="atharReflection({ max: {{ \App\Support\AtharTextLimits::PUBLIC_MAX }}, initial: @js($publicText), messages: @js(__('athar.approval.counter')), identityDisplay: @js($identityDisplay), displayName: @js($displayName), displayPosition: @js($displayPosition) })" x-init="init(); enhanceIdentityMenu()">
                <div class="athar-final-preview" dir="{{ $payloadLocale }}" lang="{{ $payloadLocale }}">
                    ...
                    <p class="athar-final-preview__name" dir="auto" x-show="attribution()" x-text="attribution()">{{ $displayName }}{{ filled($displayPosition) ? ' | '.$displayPosition : '' }}</p>
                ...
                @if (in_array($versionStatus, ['draft', 'awaiting_approval'], true))
                    <div class="athar-approval-editor">
                        ...
                        <div x-show="identityDisplay !== 'anonymous'">
                            ...
                            <input id="athar-display-name" form="athar-receipt-approval-form" name="display_name" type="text" value="{{ $displayName }}" maxlength="255" autocomplete="name" dir="auto" x-model="displayName" @if ($errors->has('display_name')) aria-invalid="true" @endif>
                            @error('display_name')<p class="athar-error" role="alert">{{ $message }}</p>@enderror
                            <label for="athar-display-position">{{ __('athar.approval.position_label') }}</label>
                            <input id="athar-display-position" form="athar-receipt-approval-form" name="display_position" type="text" value="{{ $displayPosition }}" maxlength="255" dir="auto" x-model="displayPosition" @if ($errors->has('display_position')) aria-invalid="true" @endif>
                            @error('display_position')<p class="athar-error" role="alert">{{ $message }}</p>@enderror
                        </div>
                    </div>
                @endif
            </div>
