<x-layouts.athar :title="__('athar.reflection.title')">
    <section class="athar-panel" aria-labelledby="athar-reflection-title">
        ...
        <form method="post" action="{{ localized_route('athar.submit', ['token' => request()->route('token')]) }}" class="athar-form" x-data="atharReflection({ max: {{ \App\Support\AtharTextLimits::REFLECTION_MAX }}, initial: @js($draft), messages: @js(__('athar.reflection.counter')) })" x-init="init()">
            ...
            <textarea id="freeform" name="freeform" rows="10" maxlength="{{ \App\Support\AtharTextLimits::REFLECTION_MAX }}" required autofocus dir="{{ is_rtl() ? 'rtl' : 'ltr' }}" :dir="textDirection" x-ref="field" @input="update($event.target.value)" aria-describedby="athar-reflection-meter" placeholder="{{ __('athar.reflection.note_placeholder') }}" @if ($errors->has('freeform')) aria-invalid="true" @endif>{{ $draft }}</textarea>
