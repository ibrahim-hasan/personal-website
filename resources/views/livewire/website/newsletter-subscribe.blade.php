    ...
    @if ($subscribed)
        <div class="flex justify-center"
            ...
            <div class="flex items-center gap-2 py-3">
                ...
                <span class="text-green-300 font-medium">{{ __('newsletter.subscribed_successfully') }}</span>
            </div>
        </div>
    @else
        <form wire:submit="subscribe" class="w-full">
            <div class="flex gap-3 flex-row sm:gap-0 md:rounded-[12px] rounded-[4px] border border-[#FEFEFE] md:p-3 p-[3px] md:h-[78px] h-[40px] overflow-hidden">
                <input
                    ...
                    class="flex-1 p-0 text-right md:text-xl text-sm text-light-gray-50 placeholder-light-gray-50/50 outline-none transition-colors sm:rounded-e-none sm:rounded-s-lg w-full"
                    aria-label="{{ __('Email') }}"
                    required>
                <button
                    type="submit"
                    ...
                    class="btn-primary md:rounded-[12px] rounded-[4px] p-2 md:text-xl text-sm font-normal sm:rounded-s-none sm:rounded-e-lg text-[#414042] md:min-w-[142px] min-w-[106px] disabled:opacity-70">
                    <span wire:loading.remove wire:target="subscribe">{{ __('Subscribe') }}</span>
                    <span wire:loading wire:target="subscribe" class="inline-flex items-center gap-1">
            ...
            <p class="text-sm text-yellow-50 mt-2">{{ __('newsletter.subscribe_note') }}</p>
        </form>
    @endif
</div>
