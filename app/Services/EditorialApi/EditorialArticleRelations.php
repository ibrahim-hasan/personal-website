...
final class EditorialArticleRelations
{
    ...
    public function validate(array $attributes): void
    {
        ...
        $this->validateProjects($attributes, $errors);
     ...
     * @param  array<string, list<string>>  $errors
     */
    private function validateProjects(array $attributes, array &$errors): void
    {
        if (! array_key_exists('project_keys', $attributes)) {
            return;
        }
        ...
        foreach ($keys as $index => $key) {
            ...
            if (! $this->projectCaseStudyPublicationValidator->validate($project)->isEligible()) {
