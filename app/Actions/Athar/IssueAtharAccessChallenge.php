...
class IssueAtharAccessChallenge
{
    public function handle(AtharInvitation $invitation, Request $request): string
    {
        ...
        AtharAccessChallenge::query()->create(['invitation_id' => $invitation->getKey(), 'code_hash' => AtharAccess::codeHash($code), 'expires_at' => now()->addMinutes((int) config('athar.access.code_ttl_minutes', 10)), 'requested_at' => now(), 'ip_hash' => hash_hmac('sha256', (string) $request->ip(), (string) config('app.key'))]);
